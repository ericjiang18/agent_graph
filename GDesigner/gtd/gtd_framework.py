...
from GDesigner.gtd.diffusion_model import ConditionalDiscreteGraphDiffusion
...
if __name__ == '__main__':
    ...
    # Framework Hyperparameters (matching diffusion_model.py and proxy_reward_model.py examples)
    num_nodes_example = 5
    node_feat_dim = 16  # For GraphTransformer and DiffusionModel node features
    cond_dim = 32       # For GraphTransformer and DiffusionModel condition
    time_emb_dim = 64   # For GraphTransformer time embedding
    gt_layers = 2
    gt_heads = 2
    diffusion_ts = 50   # Fewer timesteps for faster example

    # Proxy Model (dummy, for guider initialization)
    # These dimensions must match what the proxy model expects.
    proxy_node_feat_dim = node_feat_dim # Assuming proxy model uses same node features
    proxy_cond_dim = cond_dim           # Assuming proxy model uses same condition features
    proxy_gnn_hidden = 32
    proxy_gnn_layers = 2
    proxy_mlp_hidden = 64
    proxy_num_rewards = 3 # utility, cost, robustness (matches GuidedGeneration example)

    dummy_proxy = ProxyRewardModel(
    ...
    # MACP weights for guider
    macp_weights_example = {'utility': 1.0, 'cost': -0.5, 'vulnerability': -0.2}

    # Initialize GTDFramework
    print("Initializing GTDFramework with Guider...")
    gtd_system_with_guider = GTDFramework(
        node_feature_dim=node_feat_dim,
        condition_dim=cond_dim,
        time_embed_dim=time_emb_dim,
        gt_num_layers=gt_layers,
        gt_num_heads=gt_heads,
        diffusion_num_timesteps=diffusion_ts,
        proxy_reward_model=dummy_proxy,
        macp_weights=macp_weights_example,
        num_candidates_per_step=5, # K for ZO
        device=device
    )

    print("\nInitializing GTDFramework without Guider (for diffusion training)...")
    gtd_system_no_guider = GTDFramework(
        node_feature_dim=node_feat_dim,
        condition_dim=cond_dim,
        time_embed_dim=time_emb_dim,
        gt_num_layers=gt_layers,
    ...
    # 1. Test Diffusion Model Training (using gtd_system_no_guider)
    print("\n--- Testing Diffusion Model Training ---")
    # Create dummy dataset for diffusion model training
    num_train_samples_diffusion = 64 # Small dataset for example
    # (A0_truth_binary, node_features, task_condition)
    train_A0_binary = (torch.rand(num_train_samples_diffusion, num_nodes_example, num_nodes_example) > 0.7).float()
    train_node_feats_diffusion = torch.randn(num_train_samples_diffusion, num_nodes_example, node_feat_dim)
    train_conditions_diffusion = torch.randn(num_train_samples_diffusion, cond_dim)

    diffusion_dataset = TensorDataset(train_A0_binary, train_node_feats_diffusion, train_conditions_diffusion)
    # Batch size for training diffusion model
    diffusion_train_batch_size = 16
    diffusion_dataloader = DataLoader(diffusion_dataset, batch_size=diffusion_train_batch_size, shuffle=True)

    gtd_system_no_guider.train_diffusion_model(
    ...
    num_graphs_to_gen = 2
    # Dummy inputs for generation (batch size must match num_graphs_to_gen)
    gen_node_features = torch.randn(num_graphs_to_gen, num_nodes_example, node_feat_dim).to(device)
    gen_task_condition = torch.randn(num_graphs_to_gen, cond_dim).to(device)

    # Generate without guidance (using the newly "trained" diffusion model from gtd_system_no_guider)
    print("\nGenerating graphs (unguided)...")
    generated_graphs_unguided_probs = gtd_system_no_guider.generate_graphs(
        num_graphs=num_graphs_to_gen,
        num_nodes=num_nodes_example,
        node_features=gen_node_features,
        task_condition=gen_task_condition,
        use_guidance=False
    )
    print(f"Shape of unguided generated graphs (probs): {generated_graphs_unguided_probs.shape}")
    print("First unguided graph (probabilities):")
    print(generated_graphs_unguided_probs[0].round(decimals=2))

    # Generate with guidance (using gtd_system_with_guider)
    # Note: The diffusion model within gtd_system_with_guider is fresh (not trained in this script).
    # For a real scenario, you'd load trained diffusion model weights.
    # Here, we'll just use its initialized diffusion model.
    print("\nGenerating graphs (guided)...")
    # If the diffusion model in gtd_system_with_guider was not trained, results will be random-like.
    # To use the trained one: gtd_system_with_guider.diffusion_model = gtd_system_no_guider.diffusion_model
    gtd_system_with_guider.diffusion_model.load_state_dict(gtd_system_no_guider.diffusion_model.state_dict())
    print("Copied trained diffusion model weights to the guider-enabled framework.")

    generated_graphs_guided_probs = gtd_system_with_guider.generate_graphs(
        num_graphs=num_graphs_to_gen,
        num_nodes=num_nodes_example,
        node_features=gen_node_features,
        task_condition=gen_task_condition,
    ...
    print(f"Shape of guided generated graphs (probs): {generated_graphs_guided_probs.shape}")
    print("First guided graph (probabilities):")
    print(generated_graphs_guided_probs[0].round(decimals=2))

    # Binarize for easier inspection
    generated_graphs_guided_binary = (generated_graphs_guided_probs[0] > 0.5).float()
    print("First guided graph (binary):")
    print(generated_graphs_guided_binary)
    print(f"Number of edges in first guided graph: {generated_graphs_guided_binary.sum().item()}")

    print("\nGTDFramework example script finished.")
```
